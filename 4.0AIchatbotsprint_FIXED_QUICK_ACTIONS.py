...
# Allow quick action messages to act as prompt
if prompt is None and "pending_quick_prompt" in st.session_state:
...
There are {len(df)} tasks. Component expertise is as follows:\n"""
    ...
    with col1:
        if st.button("Analyze Priority Distribution"):
            st.session_state.pending_quick_prompt = "Analyze the priority distribution across team members and suggest improvements"
    st.rerun()

    with col2:
        if st.button("Check Component Balance"):
            st.session_state.pending_quick_prompt = "Check if component assignments match team member expertise"
    st.rerun()

    with col3:
        if st.button("Fix Component Mismatches"):
            st.session_state.pending_quick_prompt = "Fix component mismatches"
    st.rerun()
